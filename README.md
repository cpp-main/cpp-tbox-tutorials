...
本小节将向你展示的是：它可以同时带多个负载运行。也就是说：一个 tbox.main 框架，可以同时将多个毫不相关的业务Module运行在同一个进程里，就像是一个火车头可同时拉多节车相一样。这样，我们就可以很方便地组合多个Module。可以让所有的Module运行中一个进程里，也可以每个Module单独一个进程。

接下来，我们在上一小节的基础上，再新增一个叫“your”的Module，使之与之前叫“my”的Module一起运行在是一个进程里。  
步骤：
1. 将 my 目录复制为 your；
