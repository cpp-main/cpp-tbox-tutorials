...
|值|等级|函数|说明|用途|
|:-|:-|:-|:-|:-|
|0|FATAL|LogFatal(...)|严重错误 | 用于打印程序代码级的错误，比如程序崩馈|
|1|ERROR|LogErr(...)|错误|用于打印导致业务不能工作的严重错误。区别于FATAL，指的是非程序级错误|
|2|WARN|LogWarn(...), LogUndo()|警告|用于打印内部因素引起的异常，这种异常影响了部分功能，但其它功能依旧可用|
|3|NOTICE|LogNotice(...)|注意|用于打印外部因素引起的异常，如对方的协议格式错误、数据异常|
|4|INFO|LogInfo(...)|信息|用于打印与外部交互的信息，用于鉴别问题是外部的，还是内部的|
|5|DEBUG|LogDbg(...)|调试|用于打印内部模块间的信息，用于鉴别问题是内部的哪个模块的|
|6|TRACE|LogTrace(...), LogTag()|问题追踪|在debug时临时使用|
